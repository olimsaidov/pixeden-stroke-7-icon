...
module.exports = (function() {
  ...
  function stringify (nodes) {
    ...
    _.each(nodes, function (node) {
      sassVars.push(stringifyVar(node));
      sassClasses.push(stringifyClass(node));
    });
  ...
  function stringifyVar (icon) {
    return '$font-var-' + icon.name + ': ' + icon.value + ';';
  }

  function stringifyClass (icon) {
    return [
      '.#{$font-prefix}-' + icon.name + ':before {',
      '  content: ' + '$font-var-' + icon.name + ';',
      '}'
    ].join('\n');
  }
